...
class MemoryChatGUI(QMainWindow):
    ...
    def get_selected_model(self):
        """Ottiene il modello attualmente selezionato"""
    ...
    def on_model_changed(self, model_name):
        ...
        # Seleziona il nuovo
        self.models_config[model_name]['selected'] = True
        self.current_model = model_name
        
        # Salva configurazione
        self.save_models_config()
        
        # Aggiorna titolo finestra e status
        ...
        print(f"API Model: {self.models_config[model_name]['api_model']}")
    
    def update_window_title(self):
        """Aggiorna il titolo della finestra"""
        model_display = self.current_model
        if "Sonnet 4" in self.current_model:
    ...
    def update_model_info(self):
        """Aggiorna le info del modello corrente"""
        model_config = self.models_config[self.current_model]
    ...
    def update_chat_title(self):
        ...
        if "Sonnet 4" in self.current_model:
            ...
            title = f"🤖 Chat con {self.current_model}"
            color = "#FF9800"
            
        self.chat_title = QLabel(title)
...
Di cosa parliamo oggi?"""
            ...
            ai_name = self.current_model
            initial_msg = f"Ciao! Sono {ai_name}. Come posso aiutarti oggi?"
        
        self.add_message_to_chat(ai_name, initial_msg, "#2196F3" if "Sonnet 4" in self.current_model else "#FF9800")
...
<div style="margin: 10px 0;">
    ...
    def send_message(self):
        ...
        # Avvia thread per risposta AI con configurazione modello
        model_config = self.models_config[self.current_model]
        self.ai_thread = AIResponseThread(self.client, message, self.session_history, self.memory, model_config)
    ...
    def handle_ai_response(self, response):
        ...
        ai_name = "Claudia" if "Sonnet 4" in self.current_model else self.current_model
        color = "#2196F3" if "Sonnet 4" in self.current_model else "#FF9800"
...
Estrai solo concetti significativi, non conversazione casuale."""
        ...
        for name, config in self.models_config.items():
            if "Sonnet 4" in name:
                sonnet4_config = config
                break
        
        if not sonnet4_config:
            sonnet4_config = self.models_config[self.current_model]  # fallback
        
        self.memory_thread = AIResponseThread(self.client, prompt, [], self.memory, sonnet4_config)
    ...
    def exit_with_saving(self):
        """Salva automaticamente quando si chiude l'applicazione"""
